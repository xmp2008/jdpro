...
function ntfyNotify(text, desp) {
    // 使用方法并访问Promise对象
    sendPutRequest(NTFY_URL, { "topic": NTFY_TOPIC, "message": desp, "title": text, "priority": 4, "tags": ["rotating_light"] })
        .then(data => {
            console.log(data); // 在控制台中输出从API接收到的数据
        })
        .catch(error => {
...
// 定义名为sendPutRequest的函数，接受URL和data作为参数
        ...
        fetch(url, {
            ...
            .then(response => {
                ...
                if (response.ok) {
                    ...
                    resolve(response.json()); // 将JSON解析为JavaScript对象并将其传递给resolve函数
                } else {
